...
public class BookstoreFragment extends Fragment {
    ...
    private float mCurrentCheckedLeft;
    private final List<Fragment> mFragments = new ArrayList<>();

    private final String[] titles = {"推荐", "奇幻玄幻", "修真仙侠", "都市青春", "历史穿越", "网游竞技", "科幻灵异"};
    @Nullable
    @Override
    ...
    private void initViews(View view) {
        ...
        mViewPager = view.findViewById(R.id.bsViewPager2);
        // 初始化导航栏按钮
        ...
        mRadioGroup.setOnCheckedChangeListener((group, checkedId) -> {
            ...
            mViewPager.setCurrentItem(position, false);  // 设置页面并启用滑动动画
...
//
    ...
    private void animateToButton(RadioButton radioButton) {
        // 获取RadioButton的左边位置
        int buttonLeft = radioButton.getLeft();
        // HorizontalScrollView宽度的一半
        int scrollViewCenter = mHorizontalScrollView.getWidth() / 2;
        // 按钮宽度的一半
        int buttonCenter = radioButton.getWidth() / 2;
        // 滚动的目标位置
        int scrollToX = buttonLeft + buttonCenter - scrollViewCenter;
        // 平滑滚动到目标位置
        mHorizontalScrollView.smoothScrollTo(scrollToX, 0);
    }
    @Override
    public void onSaveInstanceState(@NonNull Bundle outState) {
        super.onSaveInstanceState(outState);
        // 保存当前选中的RadioButton状态
        outState.putInt("selectedRadioButtonId", mRadioGroup.getCheckedRadioButtonId());
    }
